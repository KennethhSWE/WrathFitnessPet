...
function initializeCharacterPreview() {
  ...
  // Clears the previous renders only once 
  if (!modelPreview.contains(renderer?.domElement)) {
    modelPreview.innerHTML = '';
  }

  //Resize the render based off users screen size 
  const modelWidth = modelPreview.offsetWidth || 300;
  const modelHeight = modelPreview.offsetHeight || 300;

  console.log('Model preview dimensions: ${modelWidth}x${modelHeight}');

  // Three.js setup
  const scene = new THREE.Scene();
  const camera = new THREE.PerspectiveCamera(75, modelWidth / modelHeight, 0.1, 1000);
  const renderer = new THREE.WebGLRenderer({ antialias: true, alpha: true });
  renderer.setSize(modelWidth, modelHeight);

  // Append renderer if not already appended 
  if (!modelPreview.contains(renderer.domElement)) {
...
async function logWorkout() {
  ...
  if (isNaN(weight) || weight <= 0) {
    document.getElementById("workout-message").innerText = showToast(
...
function updateXPProgress(level, xp, xpToNextLevel) {
  ...
  document.getElementById(
    "xp-progress-text"
  ).innerText = `${progressPercentage.toFixed(0)}% to next level`;
